...
func main() {
	...
	// ════════════════════════════════════════════════════════════
	// PARALLEL GRID PERMUTATIONS (2-Branch)
	// ════════════════════════════════════════════════════════════
...
func BuildParallelNetwork(branchTypes []nn.BrainType, combineMode string, dtype nn.DType) *nn.Network {
    ...
    for i, bt := range branchTypes {
        ...
        case nn.BrainConv2D:
            ...
            for k := range branch.Kernel { branch.Kernel[k] = (rand.Float32()*2 - 1) * InitScale }
        ...
        case nn.BrainMHA:
            numHeads := 4
            if HiddenSize%numHeads != 0 { numHeads = 2 }
    ...
    if combineMode == "concat" || combineMode == "grid_scatter" {
        ...
        for _, b := range branches {
             ...
             if b.OutputHeight > 0 {
                 ...
                 if side*side != b.InputHeight { side = 4 }
                 outSide := (side - 2) / 1 + 1
