...
func generate(net *nn.Network, tok *tokenizer.Tokenizer, vocabSize int, seed string, length int) string {
	...
	for i := 0; i < length; i++ {
		...
		temperature := 0.7 // Slightly higher temp for MHA creativity
		maxVal := out[0]
		for _, v := range out {
			if v > maxVal {
				maxVal = v
...
func main() {
	...
	fmt.Println("=== LOOM: MHA + CNN + LSTM Parallel Demo ===")
	fmt.Println("Architecture: Embedding -> Parallel[ MHA | LSTM | CNN ] -> Mixer -> Head")
	...
	// Branch Outputs
	mhaSize := embeddingSize // MHA preserves size
	lstmSize := 64           // Memory
	cnnSize := 32            // Local features

	// Total concat size
	concatSize := mhaSize + lstmSize + cnnSize // 64 + 64 + 32 = 160
	mixSize := 64
	...
	networkJSON := fmt.Sprintf(`{
		...
		embeddingSize, numHeads, // Branch 1: MHA (Seq=1 is CRITICAL)
		embeddingSize, lstmSize, // Branch 2: LSTM
		cnnSize, embeddingSize, // Branch 3: CNN (Input=Embedding, Filters=32)
		concatSize,          // LayerNorm (Normalize the concat result)
		concatSize, mixSize, // Mixer (160 -> 64)
	...
	for i := 0; i < steps; i++ {
		...
		if i > 0 && i%5000 == 0 {
			...
			fmt.Printf(" Output: \"%s...\"\n", generate(net, tok, vocabSize, "Alice", 40))
			fmt.Println("------------------------------------------------------------")
		}
