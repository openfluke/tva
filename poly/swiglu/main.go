...
func main() {
	fmt.Println("=== M-POLY-VTD Truly Exhaustive SwiGLU Benchmark ===")
	
	batchSize := 1
	inputSize := 1024
	intermediateSize := 2048 // Standard expanson for SwiGLU
	iterations := 20
	...
	// Baseline size for FP32: (GateW + UpW + DownW + Biases)
	// GateW: in * int
	// UpW:   in * int
	// DownW: int * in
	// Biases: int + int + in
	totalParams := (inputSize * intermediateSize) * 3 + (intermediateSize * 2 + inputSize)
	...
	for _, s := range allTypes {
		...
		go func(label string, dtype poly.DType) {
			...
			gradOutput := poly.NewTensor[float32](batchSize, inputSize)
			...
			expectedTrain := avgForward * 10 // SwiGLU backward is quite heavy
			simTax := 0.0
