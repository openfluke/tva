...
func generateRNNData(numSamples, inputSize, seqLen int) []SequenceSample {
	samples := make([]SequenceSample, numSamples)
	rand.Seed(42)

	for i := 0; i < numSamples; i++ {
		input := make([]float32, inputSize*seqLen)
		pattern := i % 3 // 0: start, 1: end, 2: middle

		for t := 0; t < seqLen; t++ {
			for f := 0; f < inputSize; f++ {
				baseVal := rand.Float32() * 0.2
				if (pattern == 0 && t < seqLen/3) ||
					(pattern == 1 && t >= 2*seqLen/3) ||
					(pattern == 2 && t >= seqLen/3 && t < 2*seqLen/3) {
					baseVal += 0.8
				}
				input[t*inputSize+f] = baseVal
			}
		}
...
func main() {
	fmt.Println("=== RNN Demo: Sequence Classification ===\n")

	inputSize, seqLen := 8, 16
	trainData := generateRNNData(150, inputSize, seqLen)
	testData := generateRNNData(30, inputSize, seqLen)
	...
	config := `{
		...
		"layers_per_cell": 3,
		"layers": [
			{"type": "rnn", "input_size": 8, "hidden_size": 16, "seq_length": 16},
	...
	cpuNet.Train(batches, &nn.TrainingConfig{
		Epochs: 30, LearningRate: 0.001, UseGPU: false, LossType: "cross_entropy", Verbose: true,
	})
	cpuNet.SaveModel("rnn_cpu_model.json", "rnn_cpu")

	fmt.Println("\n=== Training on GPU ===")
	gpuNet, _ := nn.LoadModelFromString(initialWeights, "rnn_init")
	gpuNet.Train(batches, &nn.TrainingConfig{
		Epochs: 30, LearningRate: 0.001, UseGPU: true, LossType: "cross_entropy", Verbose: true,
	})
	gpuNet.WeightsToCPU()
	gpuNet.GPU = false
	gpuNet.SaveModel("rnn_gpu_model.json", "rnn_gpu")

	loadedCPU, _ := nn.LoadModel("rnn_cpu_model.json", "rnn_cpu")
	loadedGPU, _ := nn.LoadModel("rnn_gpu_model.json", "rnn_gpu")
	verifyReload("CPU JSON", cpuNet, loadedCPU, testData)
	verifyReload("GPU JSON", gpuNet, loadedGPU, testData)

	safetensorsPath := "rnn_gpu.safetensors"
	...
	for _, dt := range dtypes {
		...
		fname := fmt.Sprintf("rnn_%s.safetensors", dt.Name)
	...
	os.Remove("rnn_cpu_model.json")
	os.Remove("rnn_gpu_model.json")
