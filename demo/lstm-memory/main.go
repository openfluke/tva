...
func generateLSTMData(numSamples, inputSize, seqLen int) []SequenceSample {
	samples := make([]SequenceSample, numSamples)
	rand.Seed(43)

	for i := 0; i < numSamples; i++ {
		input := make([]float32, inputSize*seqLen)
		pattern := i % 3
		markerPos := []int{2, 5, 10}[pattern]

		for t := 0; t < seqLen; t++ {
			for f := 0; f < inputSize; f++ {
				if t == markerPos && f == 0 {
					input[t*inputSize+f] = 1.0
				} else {
					input[t*inputSize+f] = rand.Float32() * 0.1
				}
			}
		}
...
func verifyReload(name string, original, reloaded *nn.Network, samples []SequenceSample) {
	...
	fmt.Printf("  Max Diff: %.9f ", maxDiff)
	if maxDiff < 1e-4 {
		fmt.Println("✓")
	} else {
		fmt.Println("✗")
...
func main() {
	fmt.Println("=== LSTM Demo: Long-term Dependencies ===\n")

	inputSize, seqLen := 8, 16
	trainData := generateLSTMData(150, inputSize, seqLen)
	testData := generateLSTMData(30, inputSize, seqLen)
	...
	config := `{
		...
		"layers_per_cell": 3,
		"layers": [
			{"type": "lstm", "input_size": 8, "hidden_size": 16, "seq_length": 16},
			{"type": "dense", "activation": "relu", "input_height": 256, "output_height": 64},
			{"type": "dense", "activation": "softmax", "input_height": 64, "output_height": 3}
		]
	}`
	...
	cpuNet.Train(batches, &nn.TrainingConfig{
		Epochs: 50, LearningRate: 0.001, UseGPU: false, LossType: "cross_entropy", Verbose: true,
	})
	cpuNet.SaveModel("lstm_cpu_model.json", "lstm_cpu")

	fmt.Println("\n=== GPU Training ===")
	gpuNet, _ := nn.LoadModelFromString(initialWeights, "lstm_init")
	gpuNet.Train(batches, &nn.TrainingConfig{
		Epochs: 50, LearningRate: 0.001, UseGPU: true, LossType: "cross_entropy", Verbose: true,
	})
	gpuNet.WeightsToCPU()
	gpuNet.GPU = false
	gpuNet.SaveModel("lstm_gpu_model.json", "lstm_gpu")

	loadedCPU, _ := nn.LoadModel("lstm_cpu_model.json", "lstm_cpu")
	loadedGPU, _ := nn.LoadModel("lstm_gpu_model.json", "lstm_gpu")
	verifyReload("CPU JSON", cpuNet, loadedCPU, testData)
	verifyReload("GPU JSON", gpuNet, loadedGPU, testData)

	safetensorsPath := "lstm_gpu.safetensors"
	...
	for _, dt := range dtypes {
		...
		fname := fmt.Sprintf("lstm_%s.safetensors", dt.Name)
	...
	os.Remove("lstm_cpu_model.json")
	os.Remove("lstm_gpu_model.json")
