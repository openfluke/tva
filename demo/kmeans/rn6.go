...
func initNetwork(m RN6TrainingMode, outputDim int) *nn.Network {
	if m == RN6ModeStandardDense {
		// Use a 2-layer grid to avoid SequentialLayer update bug in current Loom core
		net := nn.NewNetwork(RN6InputDim, 1, 1, 2)
		c1 := nn.InitDenseLayer(RN6InputDim, 64, nn.ActivationTanh)
		c2 := nn.InitDenseLayer(64, outputDim, nn.ActivationSigmoid)
	...
	if m == RN6ModeParallelKMeans {
		net := nn.NewNetwork(RN6InputDim, 1, 1, 2)
		// 2 parallel KMeans experts (concat)
		k1 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationTanh), "probabilities")
		k2 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationSigmoid), "probabilities")
		p := nn.LayerConfig{
			Type:             nn.LayerParallel,
			ParallelBranches: []nn.LayerConfig{k1, k2},
			CombineMode:      "concat",
		}
		// Final dense to map 16 inputs to classes
		final := nn.InitDenseLayer(16, outputDim, nn.ActivationSigmoid)
		net.SetLayer(0, 0, 0, p)
		net.SetLayer(0, 0, 1, final)
		net.InitializeWeights()
		return net
	}

	if m == RN6ModeParallelFilteredKMeans {
		net := nn.NewNetwork(RN6InputDim, 1, 1, 1)
		// 2 parallel KMeans experts (gated)
		k1 := nn.InitKMeansLayer(outputDim, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationTanh), "probabilities")
		k2 := nn.InitKMeansLayer(outputDim, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationSigmoid), "probabilities")
		p := nn.InitFilteredParallelLayer([]nn.LayerConfig{k1, k2}, RN6InputDim, nn.SoftmaxStandard, 1.0)
		net.SetLayer(0, 0, 0, p)
		net.InitializeWeights()
		return net
	}

	if m == RN6ModeParallelAddKMeans {
		net := nn.NewNetwork(RN6InputDim, 1, 1, 2)
		// 2 parallel KMeans experts (add)
		k1 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationTanh), "probabilities")
		k2 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationSigmoid), "probabilities")
		p := nn.LayerConfig{
			Type:             nn.LayerParallel,
		...
		final := nn.InitDenseLayer(8, outputDim, nn.ActivationSigmoid)
		net.SetLayer(0, 0, 0, p)
		net.SetLayer(0, 0, 1, final)
		net.InitializeWeights()
		return net
	...
	if m == RN6ModeParallelAvgKMeans {
		net := nn.NewNetwork(RN6InputDim, 1, 1, 2)
		// 2 parallel KMeans experts (avg)
		k1 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationTanh), "probabilities")
		k2 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationSigmoid), "probabilities")
		p := nn.LayerConfig{
			Type:             nn.LayerParallel,
	...
	if m == RN6ModeParallelGridScatterKMeans {
		net := nn.NewNetwork(RN6InputDim, 1, 1, 2)
		// 2 parallel KMeans experts (grid_scatter)
		k1 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationTanh), "probabilities")
		k2 := nn.InitKMeansLayer(8, nn.InitDenseLayer(RN6InputDim, 16, nn.ActivationSigmoid), "probabilities")
		p := nn.LayerConfig{
			Type:             nn.LayerParallel,
			...
			GridPositions: []nn.GridPosition{
				{TargetRow: 0, TargetCol: 0, TargetLayer: 0},
				{TargetRow: 0, TargetCol: 0, TargetLayer: 1},
			},
		}
		final := nn.InitDenseLayer(16, outputDim, nn.ActivationSigmoid)
		net.SetLayer(0, 0, 0, p)
