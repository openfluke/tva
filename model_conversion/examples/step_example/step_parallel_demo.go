...
func generate(net *nn.Network, tok *tokenizer.Tokenizer, vocabSize int, seed string, length int) string {
	...
	for i := 0; i < length; i++ {
		...
		for j, v := range out {
			exps[j] = float32(math.Exp(float64((v - maxVal) / 0.8))) // Increased temp slightly
			sumExp += exps[j]
		}
...
func main() {
	...
	fmt.Println("=== LOOM: Parallel Stepping Language Model (Improved) ===")
	fmt.Println("Architecture: Embedding -> Parallel[LSTM + Dense] -> Output")
	...
	// 2. Train BPE
	jsonConfig := learnBPEAndGetConfig(textData, 300) // Reduced vocab size for faster convergence
	...
	// 4. Parallel Network Construction
	// Embedding Layer (Dense) -> Parallel (LSTM + Dense) -> Output (Dense)
	hiddenSize := 64
	embeddingSize := 32
    ...
    }`, vocabSize, embeddingSize, // Embedding Layer
		embeddingSize, hiddenSize, // Branch 1: LSTM
		embeddingSize, hiddenSize, // Branch 2: Dense
		hiddenSize*2, vocabSize) // Output Layer
	...
	steps := 50000
	lr := float32(0.05) // Higher learning rate
	ptr := 0
	...
	for i := 0; i < steps; i++ {
		...
		if i > 0 && i%5000 == 0 {
			...
			fmt.Printf(" Output: \"%s...\"\n", generate(net, tok, vocabSize, "The", 15))
			fmt.Println("------------------------------------------------------------")
		}
		...
		if queue.IsFull() {
			...
			for k := range out {
				...
				// Cross-entropy gradient: prob - target
				grad[k] = prob - t
				if t > 0.5 {
	...
	fmt.Println("\n=== FINAL GENERATION ===")
	fmt.Println(generate(net, tok, vocabSize, "Alice", 40))
	fmt.Println(generate(net, tok, vocabSize, "The", 40))
}
