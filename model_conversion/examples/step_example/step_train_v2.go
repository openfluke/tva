...
func main() {
	fmt.Println("=== LOOM Stepping Neural Network v5: Max Speed Optimization ===")
	fmt.Println("3-Layer Network | No Timer Limit | Target Delay Queue")
	fmt.Println()

	// 1. Define Network Architecture
	// 3 Layers: Input(4) -> Hidden1(32) -> Hidden2(16) -> Output(3)
	// Reduced from 4 layers to start simpler
	networkJSON := `{
		"batch_size": 1,
		"grid_rows": 1,
		"grid_cols": 3,
	...
	// 3. Setup Continuous Training Loop (Max Speed)
	// No ticker, just run loop
	totalSteps := 50000 // Run for fixed steps instead of time

	// Network Depth = 3 layers
	targetDelay := 3
	targetQueue := NewTargetQueue(targetDelay)

	learningRate := float32(0.02) // Start higher
	...
	for stepCount < totalSteps {
		...
		if targetQueue.IsFull() {
			...
			// I. Logging
			if stepCount%500 == 0 { // Log less frequently since it's fast
				maxIdx := 0
				maxVal := output[0]
